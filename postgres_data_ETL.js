          ...
          if (err) {
            ...
            pool.query("SELECT setval('questions_id_seq', (SELECT MAX(id) FROM questions))", (err, res)=> {
              if (err) {
                ...
                pool.query("ALTER TABLE questions ALTER COLUMN date_written type varchar(60) USING to_char(to_timestamp(date_written/1000.0) at time zone 'UTC', 'yyyy-mm-ddThh24:mi:ss.ff3Z'), ALTER COLUMN date_written SET DEFAULT to_char(CURRENT_TIMESTAMP at time zone 'UTC', 'yyyy-mm-ddThh24:mi:ss.ff3Z')", (err, res)=> {
                  if (err) {
                    console.log(err)
                    pool.end();
                  } else {
